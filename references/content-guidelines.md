...
> 2026-07 增量补充。参考 Claude Design 官方系统提示词逆向工程（[CL4R1T4S](https://github.com/elder-plinius/CL4R1T4S/blob/main/ANTHROPIC/Claude-Design-Sys-Prompt.txt)，MIT 协议）。

### 避免自己的默认审美（自省规则）

cc-design 也有自己的默认审美倾向（与 Claude Design 类似）：

- **暖色调米白背景**（`#F5F0E8` / `oklch(0.96 0.01 80)`）
- **优雅偏正式的标题字**（Fraunces / Newsreader 等衬线）
- **土橘色暖色点缀**（`oklch(0.65 0.15 50)`）

这套风格做编辑类、酒店类、作品集类没问题。但**不管做后台管理系统、金融产品、医疗 dashboard 都默认往这个方向靠 = AI 味**。

**判据**：
- 用户主动选了这个方向 = 审美
- 用户什么都没指定，模型默认走 = slop
- 用户做的是后台/金融/医疗/工业等非编辑类场景，默认走暖色米白 = 严重 slop

**对策**：用户没指定风格时，必须走「设计方向顾问」三套并行逻辑，不能直接默认暖色米白。

### 纯白背景配纯黑文字（太生硬）

❌ `background: #FFFFFF; color: #000000;` 直接配
...
/* ✅ oklch 表达（推荐） */
background: oklch(0.98 0.005 80);  /* 暖白 */
color: oklch(0.15 0.01 80);        /* 暖黑 */
```

例外：刻意追求极端对比的 brutalism / Swiss 风格，可以纯白配纯黑——但要有明确意图，不是默认。

### 间距非 4 或 8 的整数倍

❌ 随手写 `padding: 13px` / `margin: 22px` / `gap: 7px`

除非有明确设计意图，间距应走 4 或 8 的倍数：
...
**例外**（可标注）：
- 刻意打破网格的 editorial 风格（如 Bloomberg Businessweek 的不规则栏宽）
- 有机排版（如手绘感、拼贴风的错位）
- 黄金比例推导（如 8 × 1.618 ≈ 13，可标注"黄金比例"）

### placeholder 优于垃圾实现（强化）

> 这是 cc-design 最重要的原则之一。没素材/没图标/没真数据时，**placeholder 永远优于勉强实现**。

| 没有什么 | ❌ 垃圾实现 | ✅ placeholder |
|---|---|---|
| 没插画素材 | SVG 画一个拙劣的人物/场景 | 灰色矩形 + "插画位 1200×800" 文字标签 |
| 没图标 | SVG 画一个差不多的图标 | 占位方框 + "icon: home" 文字标签 |
| 没真数据 | 编造 "10,000+ users" | "数据待补" 或问用户要 |
| 没用户 quote | 编造名人名言 | "quote 待补" 或问用户要 |
| 没产品截图 | SVG 画一个假的 UI | 灰色矩形 + "screenshot: dashboard" |

**核心**：一个诚实的 placeholder 比一个拙劣的实现强 100 倍。

### 五问测试（学 Claude Design）

页面上**每一个元素**，逐条审核：

1. **用户关心吗？** 用户真的需要这个信息吗？
2. **推动了叙事吗？** 这个元素帮用户理解/决策了吗？
3. **去掉之后页面还看得懂吗？** 如果去掉，用户会困惑吗？
4. **有更简洁的方式吗？** 能不能用更少的元素/文字表达同样意思？
5. **服务的是用户还是设计师？** 这个元素是为用户加的，还是为"让页面好看"加的？

**一个审核不通过就不应该出现在页面上**。

### 变体哲学（学 Claude Design）

每次设计给 **3+ 变体**，从保守到创意，让用户 mix and match。

**探索维度**（每次至少跨 3 个维度探索）：
- visuals（视觉风格）
- interactions（交互方式）
- color treatments（色彩处理）
- scale（尺度对比）
- fills（填充方式）
- texture（质感）
- rhythm（节奏）
- layering（层次）
- type treatments（字体处理）
- layout（布局结构）

**目标不是给用户"完美选项"，是探索尽可能多的原子变体**，让用户混搭出最适合自己的版本。

**变体原则**：
- 从 basic 开始，逐步 advanced 和 creative
- Mix by-the-book 设计（匹配现有模式）+ novel interactions（有趣布局/隐喻/视觉风格）
- 有些选项用色彩或高级 CSS，有些用 iconography，有些不用
- Remix 品牌资产和视觉 DNA——玩 scale / fills / texture / rhythm / layering / novel layouts / type treatments
...
- 用Inter？→ 换一个更有特点的
- 用紫色渐变？→ 换一个有根据的配色
- **用户没指定，想默认走暖色米白？→ 走三套并行逻辑，别默认**
- **想纯白配纯黑？→ 至少加 2% 色温**
- **间距随手写 13px？→ 走 4/8 倍数，除非有明确意图**
- **没素材想用 SVG 勉强画？→ 用 placeholder**
- **页面元素过不了五问测试？→ 删掉**
